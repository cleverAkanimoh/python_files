...
def get_back_file(cls):
    cls.back=tk.PhotoImage(file=assets_folder+"/back")
...
class GameScreen(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Crush Blackjack")
        self.geometry("800*640")
...
self.play_again_button=tk.Button(self.botton_frame,text="Play Again",command=self.play_again)
self.quit_button=tk.Button(self.bottom_frame,text="Quit Game",width=25,command=self.destroy)
...
text=self.game_state.player_score_as_text(), font=(None, 20))
